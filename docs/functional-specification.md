...
- (선택) 영감 힌트: 기본 숨김 상태, "💡 뭘 쓸지 모르겠어요" 버튼 클릭 시 영감 카드 표시 (#135)
  - 영감 카드: 10종의 정적 프롬프트 중 날짜 기반으로 선택됨
  - "다른 영감 찾기" 버튼으로 무제한 셔플 가능
  - "✕ 닫기" 버튼으로 카드 접기

#### 처리 로직
1. 기본 화면은 깔끔한 빈 캔버스로 표시된다 (영감 힌트 숨김).
2. 사용자가 "💡 뭘 쓸지 모르겠어요" 버튼을 클릭하면 영감 카드가 부드럽게 펼쳐진다.
3. 텍스트 입력 후 `교정받기` 버튼 클릭 또는 `⌘+Enter` 단축키로 제출한다.
4. 빈 텍스트 제출은 차단한다 (`text.trim()` 검증).
5. 제출 시 프론트엔드에서 `POST /api/chat`을 호출한다.
6. 요청 본문에 메시지, 모드(`diary`), 기분을 포함한다.
