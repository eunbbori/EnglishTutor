...
### FN-MISSION-001: 오늘의 단어 (챌린지 모드)

#### 입력 조건
- 챌린지 모드(`challenge`) 선택

#### 처리 로직
1. 20종의 정적 단어 목록에서 날짜 기반으로 단어를 선택한다.
   - 선택 공식: `dayOfYear % 20` (연중 일수를 단어 수로 나눈 나머지)
2. 사용자가 작성한 텍스트에서 해당 단어 사용 여부를 검사한다.
   - 정규식: `\b{단어}\b` (대소문자 무시, 단어 경계 매칭)
3. 프롬프트 변경은 하루 3회로 제한한다 (localStorage에 저장).
...
#### 출력 결과
- 오늘의 단어(영어), 뜻(한국어), 예문(영어)
- 단어 사용 완료 시 체크 표시

#### 예외 처리
- 프롬프트 변경 횟수 데이터 손상 시 초기화 (0회)
- 날짜 변경 시 횟수 자동 초기화

### FN-MISSION-002: 프롬프트 셔플 제한

#### 입력 조건
- 챌린지 모드에서 프롬프트 변경 시도

#### 처리 로직
1. localStorage에서 오늘의 셔플 데이터를 로드한다.
2. 키: `diary_shuffle_data`, 값: `{date: string, count: number}`.
3. 날짜가 오늘이 아니면 카운트를 0으로 초기화한다.
4. 카운트 < 3이면 변경 허용, 카운트 +1.
5. 카운트 >= 3이면 변경 차단 (버튼 비활성화).

#### 출력 결과
- 변경 성공: 새 프롬프트 표시 + 남은 횟수 갱신
- 변경 불가: 버튼 비활성화

#### 예외 처리
- localStorage 데이터 파싱 실패: 0으로 초기화
